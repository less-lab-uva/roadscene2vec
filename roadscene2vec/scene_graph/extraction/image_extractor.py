...
class RealExtractor(ex):
    def __init__(self, config):
        ...
        # [CM]: Select your Detectron2 segmentation model
        ...
        # [CM]: Select all objects and classes for segementation; desired classes are ultimately selected in "use_case_1_scenegraph_extraction_config.yaml"
        ...
        try:
            ...
            for path in tqdm(all_sequence_dirs):
                ...
                for frame, img in seq_images.items():
                    out_img_path = None
                    bounding_boxes, segmentation = self.get_bounding_boxes(img_tensor=img, out_img_path=out_img_path)
    ...
    def load_images(self, path):
        ...
        images = sorted([Path(f) for f in os.listdir(raw_images_loc) if isfile(join(raw_images_loc, f))], key = lambda x: int(x.stem.split(".")[0]))
    ...
    def get_seg_annotated_image(self, im):
        v = visualizer.Visualizer(im[:, :, ::-1], 
            MetadataCatalog.get(self.cfg.DATASETS.TRAIN[0]), 
            scale=1)
        out = v.draw_panoptic_seg(self.predictor(im)['panoptic_seg'][0].to('cpu'), self.predictor(im)['panoptic_seg'][1])
    ...
    def get_bounding_boxes(self, img_tensor, out_img_path=None):
        ...
        segmentation = outputs['panoptic_seg']
        bounding_boxes = outputs['instances'].pred_boxes, outputs['instances'].pred_classes, outputs['instances'].image_size, outputs['instances'].pred_masks

        return bounding_boxes, segmentation

    
    '''Returns SceneGraphDataset object containing scenegraphs, labels, and action types'''
    def getDataSet(self):
        try:
            return self.dataset
        except Exception as e:
